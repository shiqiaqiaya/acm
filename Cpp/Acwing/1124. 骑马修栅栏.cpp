...
void QAQ() {
    ...
    // 返回为 0 表示欧拉回路，> 0 表示欧拉路径
    auto [euler_type, euler_path] = [&](int n, int m, int op) {   // op 为 0 表示无向，为 1 表示有向
        ...
        if (accumulate(adj.begin() + 1, adj.end(), 0, [&](auto now, auto & x) { return now + x.size(); })) return pair{(int)-1, res};  // 边不联通
