...
遇到实在写不出正确复杂度，可以考虑随机选取，然后跑 $ T $ 次，一次的正确概率为 $ p $ ，则 $ T $ 次错误概率为 $ (1 - p)^T $ ，期望 $ \frac{1}{p} $ 次找到答案
...
**堆：** 参数不用 `vector` , `push` 返回 `point_iterator` ，配合数组记录元素堆内位置，为 `NULL` 时表示不在堆内

+ `push, join` 为 $ O(1) $ , `pop, modify, erase` 均摊 $ O(\log n) $ , 最坏 $ O(n) $

    1. `modify(point_iterator, const key)` 修改

    2. `erase(point_iterator)` 删除

    3. `join(pairing_heap & other)` 合并后 `other` 清空

**平衡树：** 以下操作均为 $ O(\log n) $

1. `find_by_order(int k)` ：需保证 $ k \in [0, size) $ 返回第 $ k + 1 $ 小的迭代器，使用时 $ k $ 需要提前 $ - 1 $

2. `order_of_key(key)` ：返回以 `cmp` 比较，小于 `key` 的个数， `key` 可以不用在树中，排名需额外 $ + 1 $

3. `join(rb_tree & other)` ：需保证键的值域不相交，合并后 `other` 清空

4. `split(const key, rb_tree & other)` ：以 `cmp` 比较，小于等于 `x` 的属于当前树，大于的属于 `other`
...
gp_hash_table<pair<string, int>, int, function<size_t(pair<string, int>)>> mp([&](const auto & k) {
    return hash<string>()(k.first) ^ hash<int>()(k.second);
});
```
...
+ **大小有关函数：**
    ...
    + **`minmax({a, b})` ：** 返回 `pair` ，`first` 最小，`second` 最大，！！使用时一定要 `minmax({a, b})`
...
+ **`decltype` 类型推导：** 用于推导类型，可以推导出引用，但是不能推导出数组

+ **`tie` 解包：** 用于解包 `pair` 或者 `tuple` ，可以用 `ignore` 忽略某个值

## 运算优先级

**高优先级：**
1. 成员访问和函数调用：`a.b`, `a->b`, `a[b]`, `a()`
2. 后置自增自减：`a++`, `a--`
3. 前置自增自减和单目运算：`++a`, `--a`, `+a`, `-a`, `!a`, `~a`
4. 类型转换：`(type)a`
5. 指针运算：`*a`, `&a`
6. `sizeof` 运算：`sizeof a`, `sizeof(type)`
7. `new` 和 `delete`：`new`, `new[]`, `delete`, `delete[]`

**中等优先级：**
8. 乘除和取模：`a * b`, `a / b`, `a % b`
9. 加减：`a + b`, `a - b`
10. 位移：`a << b`, `a >> b`
11. 关系运算：`a < b`, `a <= b`, `a > b`, `a >= b`
12. 相等运算：`a == b`, `a != b`
13. 按位与：`a & b`
14. 按位异或：`a ^ b`
15. 按位或：`a | b`
...
**低优先级：**
18. 条件运算：`a ? b : c`
19. 赋值和复合赋值：`a = b`, `a += b`, `a -= b`, `a *= b`, `a /= b`, `a %= b`, `a <<= b`, `a >>= b`, `a &= b`, `a ^= b`, `a |= b`
20. 逗号运算：`a, b`
