...
故 $ \begin{cases} x_1=y_2\\y_1=x_2-\lfloor\frac a b\rfloor y_2 \end{cases} $
...
**筛区间素数：** 使用 $ \forall p \leqslant \sqrt n $ 可筛出 $ 1 - n $ 的素数，时间复杂度 $ O(n\ln n + \sqrt{r}) , n = r - l $

```cpp
auto range_sieve = [](int l, int r) {   // 需将普通筛的 not_prime 改为 hash_table
    for (auto & p : prime) {
    	if (p * p > r) break;
        for (int j = max((int)ceil(1.0 * l / p), p) * p; j <= r; j += p) {
            not_prime[j] = true;
        }
    }
};
...
**约数个数定理：** 若 $ n=\prod \limits_{i=1}^{m}p_i^{c_i}\Rightarrow d(i)=\prod\limits_{i=1}^m(c_i+1)$
...
auto div_sieve = [](int n) {
    not_prime = vector<bool>(n + 1), d = num = vector<int>(n + 1);
    not_prime[1] = not_prime[0] = true, d[1] = 1;   // true为非质数
    for (int i = 2; i <= n; i++) {
        if (!not_prime[i]) {
            prime.push_back(i);
            num[i] = 1, d[i] = 2;   //为素数时
...
```cpp
vector<int> not_prime, prime, f, g;

auto divsum_sieve = [](int n) {
    not_prime = d = num = vector<int>(n + 1);
    not_prime[1] = d[1] = true;    // ture为素数
    for (int i = 2; i <= n; i++) {
        if (!not_prime[i]) {
            prime.push_back(i);
            g[i] = f[i] = i + 1;    //为素数时
...
$ N=\prod\limits_{p_i\in prime}p_i^{c_i}\Rightarrow $ 约数个数 $ f(n) = \prod\limits_{p_i\in prime}(c_i+1) $
...
```cpp
vector<array<int, 2>> pri;  // 0 为质因数, 1 为次幂
auto fenjie = [&](int n) {
    for (int i = 2; i * i <= n; i++) {
        if (n % i == 0) {
            pri.push_back({i, 0});
            while ((n % i) == 0) n /= i, pri.back()[1]++;
        }
    }
    if (n != 1) pri.push_back({n, 1});	//留下一个素数
};
```
...
auto fenjie = [&](int n) {
    for (auto & p : prime) {
        ...
        if (n % p == 0) {
            pri.push_back({p, 0});
            while (n % p == 0) n /= p, pri.back()[1]++;
        }
    }
    if (n != 1) pri.push_back({n, 1});
};
```
...
```cpp
vector<int> div;
auto dfs = [&](auto && dfs, int pos = 0, int now = 1) -> void {
    if (pos >= pri.size()) {
        div.push_back(now);
        return;
    }
    for (int j = 0, t = 1; j <= pri[pos][1]; j++, t *= pri[pos][0]) {
        dfs(dfs, pos + 1, now * t);
    }
};
```
