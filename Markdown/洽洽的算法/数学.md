...
(ii) $x_1,k_1为方程ax+nk=b一组解\Rightarrow 任意解为\cases{x=x_0+\frac{nt}{\gcd{(a, n)}}\\k=k_0-\frac{at}{\gcd{(a,n)}}},t\in \Z$
...
**作用：**$求\gcd(n_i) = 1(i = 1, 2, \cdots, n)且\cases{x \equiv a_1\pmod {n_1}\\x\equiv a_2\pmod {n_2}\\\vdots\\ x\equiv a_n\pmod{n_k}}$
...
for (int i = 2; i * i <= n; i++) {
    if (vis[i]) {
        preim.push_back(i);
        for (int j = i * i; j <= n; j += i) {	//因为从2到i-1的倍数之前都筛过
            vis[j] = false;
        }
    }
}
...
$\mu = \cases{1&n=1\\0&n的一个质因子次数超过一次\\(-1)^k&k为n的本质不同质因子个数}$
...
$斐波那契数列a_n=\cases{1&n=1,n=2\\a_{n-1}+a_{n-2}&n>2}$

$记1\times 2阶矩阵F(n)=[\matrix{a_n&a_{n-1}}]\Rightarrow F(n-1)=[\matrix{a_{n-1}&a_{n-2}}]$

$试设矩阵B使得F(n-1)\times B=F(n-1)\Rightarrow B=\left[\matrix{1&1\\1&0}\right]$
