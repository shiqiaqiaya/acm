...
多项式$Hash$函数分$f(s)=\sum\limits_{i=1}^ls[i]\times b^{l-i}\pmod M与f(s)=\sum\limits_{i=1}^ls[i]\times b^{i-1}\pmod M$

**多次询问子串哈希**可预处理出一个$b$进制数$mod\ M$的数组,$f(s[l\dots s]=f(r)-f(l-1)\times b^{r-l+1}$,复杂度$O(1)$
...
struct Hash {	//双哈希
    ...
    Hash(string & s) {	// 传入的 s 必须重 1 开始
        pre = pw = vector<array<int, len>>(s.size());
        for (int _ = 0; _ < len; _++) {
            pw[0][_] = 1;
        }
    ...
    array<int, len> query(int l, int r) {
        array<int, len> res;
        for (int _ = 0; _ < len; _++) {
            res[_] = ((pre[r][_] - pre[l - 1][_] * pw[r - l + 1][_] % mod[_]) % mod[_] + mod[_]) % mod[_];
        }
        return res;
    }
};
...
auto prefix_function = [&](string & s) {	//s的下标从1开始
