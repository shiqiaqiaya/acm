...
3. **时间复杂度(单调队列)：**$O(nr_i\lfloor \frac v {v_i}\rfloor) = O(nv)$
   ...
   $\Large 令r = v \mod v_i, 对max的k进行换元成\cases{v - k \times v_i\rightarrow k \times v_i + r\\k \times w_i \rightarrow (\lfloor \frac v {v_i} \rfloor - k) \times w_i\\v\rightarrow \lfloor \frac v {v_i} \rfloor \times v_i + r}$
